...
from nanobot.agent.memory_backend import create_memory_backend
...
from nanobot.config.schema import AgentDefaults, MemoryConfig, ModelPresetConfig
...
class AgentLoop:
    ...
    def __init__(
        ...
        self.memory_backend = create_memory_backend(
            workspace, memory_config or MemoryConfig(), self.runtime_resolver.runtime,
        )
    ...
    async def _run_agent_loop(
        ...
        *streaming*: request incremental output from the runner.
        ...
        include_memory = not ephemeral and (session is None or session.policy.persist)
        retrieved_memory = ""
        if include_memory and transcript_input.current_message:
            self.memory_backend.update_runtime(
                default_model=runtime.model,
                api_key=getattr(runtime.provider, "api_key", None),
                api_base=getattr(runtime.provider, "api_base", None),
            )
            retrieved_memory = self.memory_backend.retrieve_context(
                transcript_input.current_message,
                session_key=active_session_key,
                recent_history=transcript_input.history,
            )
    ...
    async def _persist_turn(self, ctx: TurnContext) -> None:
        ...
        self.sessions.save(session)
        if not ctx.ephemeral:
            if ctx.kind is TurnKind.USER and ctx.final_content and ctx.stop_reason not in {"error", "tool_error"}:
                runtime = ctx.require_runtime()
                self.memory_backend.update_runtime(
                    default_model=runtime.model,
                    api_key=getattr(runtime.provider, "api_key", None),
                ...
                self.memory_backend.add_turn(
                    ctx.msg.content, ctx.final_content, session_key=ctx.session_key,
                )
            await self.runtime_event_publisher.session_turn_persisted(
                ctx.msg,
                ctx.session_key,
