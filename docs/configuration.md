...
Install the optional backend with `uv pip install -e ".[memoryos]". File-backed
Dream memory stays active alongside retrieval, including when the optional backend
is unavailable. Transient/private turns neither retrieve nor store MemoryOS data;
token estimation does not perform retrieval. Explicit MemoryOS credentials remain
fixed when the chat provider or model changes.

This fork retains upstream WhatsApp support but leaves the channel disabled by
default. Keep `"whatsapp": {"enabled": false}` under `channels` in your config to
disable it explicitly; no channel source files need to be removed.
